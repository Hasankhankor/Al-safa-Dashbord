...
interface DataTableProps<TData> {
  table?: TableType<TData>; // table is optional
...
export default function DataTable<TData>({
  ...
  // Fallback if `table` is undefined
  if (!table) {
    return <div>Loading...</div>;
  }

  // Safely access the header groups and rows
  const headerGroups = table.getHeaderGroups?.() ?? [];  // Safe fallback
  const rows = table.getRowModel()?.rows ?? [];  // Safe fallback
